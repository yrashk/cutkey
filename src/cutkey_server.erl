...
max_jobs(Limit) when is_integer(Limit) andalso Limit > 0 ->
    gen_server:call(?SERVER, {set_limit, Limit}, infinity).

start_link() ->
    Limit = case application:get_env(cutkey, max_jobs) of
...
init(Limit) when is_integer(Limit) andalso Limit > 0 ->
...
process_queue(#state{requests = Reqs, queue = Q, limit = Limit} = State) ->
    case Reqs =:= [] orelse length(Reqs) > Limit of
	true ->
	    case queue:out(Q) of
		{{value, Job}, NewQ} ->
...
start_job({Ref, From, {gen_rsa, Bits, E}},
    ...
    case cutkey_drv:gen_rsa(Port, Ref, Bits, E) of
	ok ->
